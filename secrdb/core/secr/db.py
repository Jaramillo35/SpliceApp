...
from secrdb.config import SECR_DB_PATH as DB_PATH
from secrdb.core.secr.enrich import filter_dtcr_mapping_for_family
from secrdb.core.secr.identity import FIRST_SEQUENCE_NUMBER, scope_key
from secrdb.core.secr.parse import ParsedSecr, parse_secr_bytes
...
#: v4 records where a circuit terminates. ``CKT FROM/To (DNUM | CAV)`` used to
#: be stored only as opaque text in ``old_value``/``new_value``, so a connector
#: that appeared solely as a circuit endpoint was invisible to every query. A
#: DNUM is an FCA-CNUM — verified across the validation set, 619 of 619 DNUMs
#: also appear as a connector — so these columns join circuits to connectors.
#: Additive: existing rows keep NULL endpoints until their SECR is re-imported.
_MIGRATION_V4 = """
CREATE INDEX IF NOT EXISTS ix_change_from_dnum ON secr_change (from_dnum);
CREATE INDEX IF NOT EXISTS ix_change_to_dnum   ON secr_change (to_dnum);
"""
...
#: v5 keeps the *previous* endpoint as well as the current one. A circuit
#: leaving a pin is a change to that pin, and v4 recorded only where the
#: circuit ended up — so the row that moved a circuit off ``X501A|7`` was not
#: attributed to X501A at all. 87 of the 346 endpoint changes in the validation
#: set lost their old connector this way.
_MIGRATION_V5 = """
CREATE INDEX IF NOT EXISTS ix_change_from_dnum_old ON secr_change (from_dnum_old);
CREATE INDEX IF NOT EXISTS ix_change_to_dnum_old   ON secr_change (to_dnum_old);
"""
...
#: v6 stores DTCR Matching Reports so they are uploaded once per
#: program + model year + phase instead of re-attached to every SECR. The
#: original workbook is kept alongside the parsed rows: enrichment consumes the
#: file, the dashboard queries the rows, and neither has to re-derive the other.
_MIGRATION_V6 = """
CREATE TABLE IF NOT EXISTS dtcr_report (
...
-- One report per scope. Re-uploading replaces it, so the library cannot drift
-- into two reports that disagree about the same program/year/phase.
CREATE UNIQUE INDEX IF NOT EXISTS ux_dtcr_report_scope
    ON dtcr_report (program, model_year, phase);

CREATE TABLE IF NOT EXISTS dtcr_report_row (
    id                    INTEGER PRIMARY KEY,
    report_id             INTEGER NOT NULL REFERENCES dtcr_report(id) ON DELETE CASCADE,
...
-- A DTCR can name several harness families in one cell ("BODY_RIGHT,
-- BODY_LEFT"). Exploding them here is what makes "how many DTCRs hit BODY_LEFT"
-- a count rather than a string search.
CREATE TABLE IF NOT EXISTS dtcr_report_family (
    id             INTEGER PRIMARY KEY,
    report_id      INTEGER NOT NULL REFERENCES dtcr_report(id) ON DELETE CASCADE,
    row_id         INTEGER NOT NULL REFERENCES dtcr_report_row(id) ON DELETE CASCADE,
    dtcr_number    TEXT NOT NULL,
    harness_family TEXT NOT NULL
);

CREATE INDEX IF NOT EXISTS ix_dtcr_row_report   ON dtcr_report_row (report_id);
CREATE INDEX IF NOT EXISTS ix_dtcr_row_number   ON dtcr_report_row (dtcr_number);
CREATE INDEX IF NOT EXISTS ix_dtcr_row_cnum     ON dtcr_report_row (cnum);
CREATE INDEX IF NOT EXISTS ix_dtcr_family_rep   ON dtcr_report_family (report_id);
CREATE INDEX IF NOT EXISTS ix_dtcr_family_name  ON dtcr_report_family (harness_family);
"""

#: The four (connector, cavity) pairs one circuit change touches: each end,
#: before and after. A pin query must test a connector and a cavity **on the
#: same side of the same state** — testing ``(from_dnum OR to_dnum)`` against
#: ``(from_cav OR to_cav)`` matches a row where the connector is on one end and
#: the cavity belongs to the other, which is a different pin entirely.
_ENDPOINT_PAIRS = (
    ("from_dnum", "from_cav"),
    ("to_dnum", "to_cav"),
    ("from_dnum_old", "from_cav_old"),
    ("to_dnum_old", "to_cav_old"),
)


def endpoint_clause(cavity: str = "") -> str:
    """SQL matching a connector — optionally one cavity — at any of its ends."""
    if cavity:
        return "(" + " OR ".join(
            f"(c.{dnum} = ? AND c.{cav} = ?)" for dnum, cav in _ENDPOINT_PAIRS
        ) + ")"
    return "(" + " OR ".join(
        f"c.{dnum} = ?" for dnum, _cav in _ENDPOINT_PAIRS
    ) + ")"


def endpoint_params(connector: str, cavity: str = "") -> List[str]:
    """Bind values for :func:`endpoint_clause`, in the same order."""
    dnum = connector.strip().upper()
    if cavity:
        cav = cavity.strip().upper()
        return [value for _pair in _ENDPOINT_PAIRS for value in (dnum, cav)]
    return [dnum] * len(_ENDPOINT_PAIRS)
...
def init_db(db_path: Optional[Path] = None) -> None:
    ...
    with connect(db_path) as conn:
        ...
        if current < 4:
            existing = {
                row["name"] for row in conn.execute("PRAGMA table_info(secr_change)")
            }
            for column, definition in _V4_CHANGE_COLUMNS.items():
                if column not in existing:
                    conn.execute(
                        f"ALTER TABLE secr_change ADD COLUMN {column} {definition}"
                    )
        ...
        if current < 5:
            ...
            for column, definition in _V5_CHANGE_COLUMNS.items():
                if column not in existing:
                    conn.execute(
                        f"ALTER TABLE secr_change ADD COLUMN {column} {definition}"
                    )
        ...
        for change in record.get("changes", []):
            conn.execute(
                ...
                INSERT INTO secr_change (
                    ...
                    from_dnum, from_cav, to_dnum, to_cav,
                    from_dnum_old, from_cav_old, to_dnum_old, to_cav_old
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    ...
                    change.get("from_dnum"), change.get("from_cav"),
                    change.get("to_dnum"), change.get("to_cav"),
                    change.get("from_dnum_old"), change.get("from_cav_old"),
                    change.get("to_dnum_old"), change.get("to_cav_old"),
                ),
            )
...
def reserve_next_secr_number(
    ...
    table rather than being derived from ``MAX(secr_number) + 1`` so that a
    number stays spent while the SECR that holds it exists — two SECRs must
    never share an identity.

    Deleting the *most recent* SECR does give its number back (see
    :func:`_reclaim_sequence`): deletion is how a mistaken generation is undone,
    and skipping a number for a SECR that no longer exists helps nobody. A
    number is only reclaimed when nothing was issued after it.
...
def delete_secr(secr_id: int, db_path: Optional[Path] = None) -> bool:
    ...
    with connect(db_path) as conn:
        row = conn.execute(
            "SELECT secr_number, version, scope_model_year, scope_phase,"
            "       secr_sequence_number FROM secr WHERE id = ?",
            (secr_id,),
        ).fetchone()
        if row is None:
            return False
        ...
        scope = (
            row["scope_model_year"],
            row["scope_phase"],
            row["secr_sequence_number"],
        )
        conn.execute("DELETE FROM secr WHERE id = ?", (secr_id,))
        reclaimed = _reclaim_sequence(conn, *scope)
        detail = f"{change_count} change record(s) removed with it"
        if reclaimed:
            detail += f"; number {reclaimed} is free again"
        _log_audit(
            conn,
...
def _reclaim_sequence(
    conn: sqlite3.Connection,
    model_year: Any,
    phase: Any,
    sequence_number: Any,
) -> Optional[int]:
    """Give a deleted SECR's number back, when nothing was issued after it.

    Deleting is how an engineer undoes a generation they did not mean to make,
    and leaving a hole at the end of the range means the next real SECR skips
    a number for no reason — reported from the field after deleting 1006 and
    1007 and being handed 1008.

    Only the *trailing* numbers come back. If 1005 is deleted while 1006 still
    exists, 1005 stays spent: reissuing a number from the middle of a range
    while later ones are in circulation is how two different SECRs end up
    sharing an identity.

    Returns the number the sequence was rolled back to, or ``None``.
    """
    if sequence_number is None or not model_year or not phase:
        return None      # imported SECRs carry no sequence number
    scope_year, scope_phase = scope_key(str(model_year), str(phase))

    highest = conn.execute(
        "SELECT MAX(secr_sequence_number) FROM secr"
        " WHERE scope_model_year = ? AND scope_phase = ?"
        "   AND secr_sequence_number IS NOT NULL",
        (scope_year, scope_phase),
    ).fetchone()[0]
    next_free = (int(highest) + 1) if highest is not None else FIRST_SEQUENCE_NUMBER

    row = conn.execute(
        "SELECT next_number FROM secr_sequence WHERE model_year = ? AND phase = ?",
        (scope_year, scope_phase),
    ).fetchone()
    if row is None or next_free >= int(row["next_number"]):
        return None      # a later number is still issued; nothing to give back

    conn.execute(
        "UPDATE secr_sequence SET next_number = ?, updated_at ="
        " datetime('now','localtime') WHERE model_year = ? AND phase = ?",
        (next_free, scope_year, scope_phase),
    )
    return next_free
...
def search_secrs(
    ...
    *,
    ...
    if dtcr:
        clauses.append(
            "(" + DTCR_IN_LIST_SQL.format(column="s.dtcr_numbers")
            + " OR EXISTS (SELECT 1 FROM secr_change c WHERE c.secr_id = s.id"
            "            AND " + DTCR_IN_LIST_SQL.format(column="c.dtcr_number") + ")"
            " OR EXISTS (SELECT 1 FROM secr_dtcr d WHERE d.secr_id = s.id"
            "            AND d.dtcr_number = ?))"
        )
        params.extend([dtcr.strip(), dtcr.strip(), dtcr.strip()])
...
#: Match one DTCR against a column that may hold a comma-separated *list*.
#:
#: ``secr_change.dtcr_number`` holds every DTCR named on that row — one comment
#: routinely names two ("DTCR 50298 AND 50318"), and the matching report ties
#: several DTCRs to one CNUM. A plain ``= ?`` matched only rows carrying a
#: single DTCR, so a number that happened to be written second was invisible.
#:
#: Wrapping both sides in commas makes the test exact per element: ``50318``
#: matches ``"50298, 50318"`` but never ``"503181"``. Spaces are stripped so
#: ``"50298, 50318"`` and ``"50298,50318"`` behave identically.
DTCR_IN_LIST_SQL = (
    "(',' || REPLACE(COALESCE({column}, ''), ' ', '') || ',') LIKE ('%,' || ? || ',%')"
)
    ...
    *,
    ...
    for column, value in (
        ...
        if value:
            ...
            params.append(value)
    if dtcr_number:
        clauses.append(DTCR_IN_LIST_SQL.format(column="c.dtcr_number"))
        params.append(dtcr_number.strip())
    if object_id:
    ...
    # A connector shows up in two roles: as the object that changed, and as the
    # DNUM a circuit terminates on. They are the same namespace — every DNUM in
    # the validation set is also an FCA-CNUM — so `connector` asks the question
    # an engineer actually means: "anything involving this connector".
    # `cnum` above stays object-only, so the Browse and Dashboard connector
    # charts keep counting exactly what they counted before.
    if connector:
        term = connector.strip()
        clauses.append(
            "((c.object_type = 'connector' AND c.object_id LIKE ?) OR "
            + endpoint_clause(cavity)
            + ")"
        )
        params.append(f"{term}%")
        params.extend(endpoint_params(term, cavity))
    elif endpoint:
        clauses.append(endpoint_clause(cavity))
        params.extend(endpoint_params(endpoint, cavity))
    elif cavity:
        # A cavity on its own is ambiguous across connectors, but asking for it
        # should still narrow rather than silently do nothing.
        clauses.append(
            "(" + " OR ".join(f"c.{cav} = ?" for _d, cav in _ENDPOINT_PAIRS) + ")"
        )
        params.extend([cavity.strip().upper()] * len(_ENDPOINT_PAIRS))
    if bulletin:
        clauses.append("s.bulletin_numbers LIKE ?")
    ...
    if term:
        clauses.append(
            ...
            " OR c.dtcr_number LIKE ? OR " + endpoint_clause()
            + " OR c.se_comment LIKE ?"
            " OR s.secr_number LIKE ? OR s.harness_family LIKE ?"
            " OR s.bulletin_numbers LIKE ?)"
        )
        params.extend(
            [f"{term}%"] * 4
            + endpoint_params(term)
            + [f"%{term}%"] * 4
        )
