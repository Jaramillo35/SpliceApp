...
def _load_baseline_minutes_by_workflow() -> dict[str, int]:
    baseline_map: dict[str, int] = {}

    baseline_file = os.getenv("METRICS_BASELINES_PATH")
    if not baseline_file:
        baseline_file = str(Path(__file__).resolve().parents[1] / "data" / "impact_baselines.json")

    try:
        raw = json.loads(Path(baseline_file).read_text(encoding="utf-8"))
        if isinstance(raw, dict):
            for key, value in raw.items():
                if value is None:
                    continue
                baseline_map[str(key)] = int(value)
    except Exception:
        pass

    env_raw = os.getenv("METRICS_BASELINE_MINUTES_JSON")
    if env_raw:
        try:
    ...
    try:
        secret_payload = st.secrets.get("METRICS_BASELINE_MINUTES_BY_WORKFLOW")
...
class WorkflowRunContext:
    ...
    def complete(self, *, output_generated: bool, output_file_count: int | None = None) -> None:
        ...
        self.tracker._record_automatic_feedback(self.workflow_id, self.run_id, processing_seconds)
...
class MetricsTracker:
    def __init__(self, storage: MetricsStorage) -> None:
        ...
        self.baseline_minutes_by_workflow = _load_baseline_minutes_by_workflow()
    ...
    def _record_automatic_feedback(self, workflow_id: str, workflow_run_id: str, processing_seconds: float) -> None:
        baseline_minutes = self.baseline_minutes_by_workflow.get(workflow_id)
        feedback_payload = {
            "id": str(uuid.uuid4()),
            ...
            "baseline_manual_touchpoints": None,
            "remaining_manual_touchpoints": None,
            "manual_touchpoints_eliminated": None,
            "user_reported_errors_prevented": None,
            "usefulness_rating": None,
            "non_confidential_feedback": None,
