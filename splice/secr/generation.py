...
(:mod:`splice.secr.generate`); this module only decides identity, numbering,
...
from splice.secr.numbering import auto_enrich_secr
...
class NewSecrPlan:
    ...
    def can_generate(self) -> bool:
        """Generation is blocked while anything is missing or ambiguous."""
...
def plan_new_secr(
    ...
    *,
    ...
    number, version and filename. Any missing or ambiguous value lands in
    ``problems`` and blocks generation — nothing is guessed.
    """
    extracted = extract_metadata_from_def(def_bytes, def_filename)
    metadata = extracted.metadata
    problems = list(extracted.conflicts) + validate_metadata(metadata, change_type)
...
def _enrich(
    ...
    Runs :func:`splice.secr.numbering.auto_enrich_secr` — the same call the SECR
...
def plan_secr_update(
    ...
    *,
    ...
    problems = list(extracted.conflicts) + validate_metadata(
        new_metadata, change_type
    )
