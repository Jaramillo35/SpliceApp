...
def _harness_from_def_filename(def_filename: str) -> str:
    ...
    Mirrors how :func:`splice.secr.generate.create_secr_bytes` reads it, so the
...
def extract_metadata_from_def(
    ...
    across phases (``X0_A`` vs ``X1_A``) — and is reported as a note, not a
    conflict. A difference in program, model year or harness family is not
    normal and is raised as a conflict for the engineer to resolve.
    """
    old_text = new_text = ""
    try:
        ...
        return ExtractedMetadata(
            metadata=SecrMetadata(),
            conflicts=[f"The DEF compare file could not be read: {exc}"],
        )
    ...
    if filename_harness:
        if not harness:
            ...
            conflicts.append(
                ...
                f"'{filename_harness}' but the DEF_New identifier says '{harness}'."
            )
        ...
        for label, name in (
            ...
            if old_value and new_value and old_value != new_value:
                conflicts.append(
                    ...
                    f"NEW is '{new_value}'. This normally means the wrong pair of "
                    f"DEFs was compared."
                )
