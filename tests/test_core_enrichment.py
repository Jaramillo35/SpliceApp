...
from secrdb.core.secr import api, db as secr_db, generation
from secrdb.core.secr.generation import assign_dtcrs_to_cnums, build_cnum_dtcr_map
...
def test_assigned_dtcrs_are_queryable(db_path: Path) -> None:
    ...
    The row stores both DTCRs the report mapped to this CNUM, and **either one
    on its own** finds it — an engineer asks about ``50319``, not about the
    stored pair.
    """
    _generate(db_path, build_matching_report(MATCHING_ROWS), se_comment="")

    for one in ("49754", "50319"):
        changes = secr_db.find_changes(dtcr_number=one, db_path=db_path)
        assert changes and changes[0]["object_id"] == "SD401", (
            f"DTCR {one} did not find the change it is stored on"
        )


def test_a_dtcr_filter_does_not_match_a_longer_number(db_path: Path) -> None:
    """`5031` and `503190` must not match a row storing `50319`."""
    _generate(db_path, build_matching_report(MATCHING_ROWS), se_comment="")

    assert secr_db.find_changes(dtcr_number="5031", db_path=db_path) == []
    assert secr_db.find_changes(dtcr_number="503190", db_path=db_path) == []
